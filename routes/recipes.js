...
These are leftovers that will be WASTED if not used. Use these FIRST in as many recipes as possible. Build at least 2-3 recipes around these leftovers combined with the sale items. Mark leftover items as type "ON_HAND". For each recipe, include "usesLeftovers": true if it uses any leftover items, otherwise "usesLeftovers": false.`
    ...
    if (activeDiets.length) {
      ...
      dietNote = `\n\nSTRICT DIETARY RESTRICTIONS — THESE ARE ABSOLUTE AND MUST NEVER BE VIOLATED:\n${rules.join("\n")}\n\nI have already removed sale items that violate these restrictions from the list below. Do NOT add any ingredients that violate these rules. Do NOT suggest meat/fish/dairy substitutions if those are restricted. Every single recipe must fully comply.\n\nTHESE RESTRICTIONS OVERRIDE THE PANTRY-STAPLE INSTRUCTIONS BELOW. The pantry-staple guidance tells you to add common staples as needed — but a "staple" that violates the restrictions above is FORBIDDEN. Specifically:\n- Do NOT add rice, pasta, noodles, bread, buns, tortillas, flour, or sugar as a pantry staple if the restrictions forbid them.\n- Do NOT put a forbidden food in a SERVING SUGGESTION. Never write "serve over rice", "serve on a bun", "serve with tortillas", or similar.\n- Do NOT put a forbidden food in a garnish, side dish, topping, or optional ingredient.\n- If a dish would normally be served over rice or in a tortilla, serve it a compliant way instead (over cauliflower rice, in lettuce wraps, over roasted vegetables, or on its own).\nA recipe that violates these rules ANYWHERE — including in a side, garnish, or serving suggestion — will be discarded entirely.`;
    }
