// ── Lucide inline-SVG icon helper ───────────────────────────────────────────
// Returns a raw Lucide SVG string so icons render at innerHTML time with no
// post-render createIcons() call. Inherits color via currentColor and sizes to
// 1em by default. Only the UI-chrome icons we actually use are included.
const _ICON_PATHS = {
...
function icon(name, opts) {
  ...
  return `<svg${cls} width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-0.125em;flex-shrink:0;${(opts && opts.style) || ''}">${p}</svg>`;
}
...
async function renderStoreBrands() {
  ...
  updateStoresBtnState();
}
function updateStoresBtnState() {
  ...
  if (count === 0) {
    btn.disabled=true;
    btn.textContent="View Deals →";
    btn.setAttribute("aria-label","View Deals (select at least one store first)");
    btn.setAttribute("title","Select at least one store to continue");
    screen?.classList.remove("has-selection");
  } else {
    btn.disabled=false;
    btn.textContent=`View Deals from ${count} Store${count>1?"s":""} →`;
    btn.setAttribute("aria-label",`View deals from ${count} selected ${count===1?"store":"stores"}`);
...
function updateKrogerBtnState() {
  ...
  if (!state.selectedKrogerId) {
    btn.disabled=true;
    btn.setAttribute("aria-label","Continue (select a Kroger location first)");
    btn.setAttribute("title","Select a Kroger location to continue");
    screen?.classList.remove("has-selection");
  } else {
    btn.disabled=false;
    btn.setAttribute("aria-label","Continue with selected Kroger location");
    btn.removeAttribute("title");
    screen?.classList.add("has-selection");
  }
}
...
if (!localStorage.getItem("dishcount_cookies_accepted")) {
  ...
  cb.innerHTML = '<span>\u{1F36A} Dishcount uses cookies for analytics and to keep you signed in.</span><button onclick="localStorage.setItem(\'dishcount_cookies_accepted\',\'true\');document.body.style.paddingBottom=\'\';this.parentElement.remove()" style="background:#2d6a4f;color:#fff;border:none;border-radius:8px;padding:6px 16px;font-size:13px;font-weight:700;cursor:pointer;font-family:\'DM Sans\',sans-serif;white-space:nowrap">Got it</button>';
  document.body.appendChild(cb);
  // Add bottom padding equal to banner height + small gap so the banner doesn't
  // overlap page content (e.g., the bottom store row in the store list at long
  // viewports). Padding is reset in the dismiss handler above.
  document.body.style.paddingBottom = (cb.offsetHeight + 8) + "px";
}
