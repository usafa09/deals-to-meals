...
// table; meijer and lidl had the weeklyad URL but no flag, so Vision OCR ran
// against a page whose rows were already machine-readable. Neither failure was
// loud. Assert the invariant at module load so the next drift cannot boot.
...
for (const [k, u] of Object.entries(IGROCERYADS_STORES)) {
  if (u.includes("weeklyad.us.com") && !TABLE_SOURCED[k])
    throw new Error(`${k} is on weeklyad but not TABLE_SOURCED`);
}
