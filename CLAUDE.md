...
- **The wait-state ticker ranks on food words, not product forms.** `_TICKER_PROTEIN` and `_TICKER_PRODUCE` (`public/app.js:2245-2246`) decide which deal the "Working with…" line names first. They were word-anchored on 2026-08-31, which fixed matching *accidents* (Hamburger Buns, Ground Coffee, Pepperoni Pizza, Popcorn, Applewood Bacon), but they still only ask "does this name contain a food word" — never "is this product actually that food". So a processed item whose name legitimately contains the word still ranks in that food's tier. **Concretely: "Store Baked Steak Rolls" is now position 1 on ShopRite — a bakery item ranking as protein. That is correct behaviour for the pattern and wrong for the intent.** Same class: `Apple Juice`, `Corn Tortillas`, `Key Lime Pie`, `Beef Hot Dogs`, `Steak Sauce`, `Crunch Berries Cereal`. Display-only — `tickerDeals()` affects the order items are *named* during the wait and never what the recipe model receives, so the worst case is an unappetising first frame on camera, not a wrong recipe. Logged 2026-08-31, deliberately not fixed — anchoring is a pattern tweak, this needs a classifier. The real fix is a product-form demotion list (juice, soda, pizza, pie, cake, cereal, tortilla, chips, candy, sauce, dressing, coffee, tea, buns, rolls, snack) checked *before* tier assignment, dropping a row to "other" whatever food word it contains. It must be validated the same way the anchoring pass was — diff old vs new tiers across all ~5,800 unique names in `deal_cache` and read both directions — because a careless form list demotes real food (`Fresh Blueberries - 1 PT Clamshell`, `brioche` on a genuine bakery deal) as easily as it demotes cereal.
